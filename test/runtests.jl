...
realvalue(x::SymLog) = symexp(symlog(x))
...
@testset "SymmetricLogarithmicNumbers" begin
    ...
    @testset "SymLog construction" begin
        xs = [-3.5, -1.0, -0.25, 0.0, 0.25, 1.0, 3.5]
        for x in xs
            hx = SymLog(x)
            @test hx isa SymLog{Float64}
            @test isapprox(realvalue(hx), x; atol=eps(x)*10, rtol=0)
        end

        @test SymLog{Float32}(2.0) isa SymLog{Float32}
        @test isapprox(realvalue(SymLog{Float32}(2.0)), 2f0; atol=eps(Float32)*10, rtol=0)
    end

    @testset "Arithmetic" begin
        pairs = [(-3.0, 0.5), (-1.5, -0.25), (0.75, 1.5), (2.0, -4.0)]
        for (x, y) in pairs
            hx, hy = SymLog(x), SymLog(y)
    ...
    @testset "Logarithms" begin
        ...
        @test_throws DomainError log(SymLog(-0.5))
    end

    @testset "Zero and identity" begin
        zero_val = zero(SymLog{Float64})
        one_val = one(SymLog{Float64})
